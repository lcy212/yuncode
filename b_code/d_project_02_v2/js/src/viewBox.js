...
// 1. 마지막 요소를 복제하여 첫번째 요소 앞에 위치시킨다
const elSlideLast = elSlideCvt.at(-1);                // 마지막 요소 선택 at(-1) : 뒤에서 첫번째
...
elSlideLiReCvt.forEach((li, idx) => {                 // li가 여러개이기때문에 forEach로 각각 스타일을 설정해줘야함
...
// 함수 -----------------------------------------------------------------------------------------------------------------
// setIimeout 간략화 함수
const fnDelay = async (ms = 0) => {                     // 순서대로 진행하려면 async가 필요하고 return await와 세트
...
// 다음버튼 함수
const fnNextAni = async () => {                         // async를 함수안에 넣으려면 함수도 async를 사용해야함
  if(SLIDE_COUNT >= slideLen){                          // 마지막 요소에서 첫번째 요소로 이동하는 if문
    SLIDE_COUNT = 0;
    ulStyle.transition = null;                          // 마지막요소에서 복제요소로 이동할때 효과가 없어야함
    ulStyle.left = 100 + '%';                           // 마지막 요소로 이동
  }
  await fnDelay();
  ulStyle.transition = `left ${TIME_OPTION}ms linear`;  // 복제요소로 이동 후 다음 요소를 위해 효과를 넣음

...
const elBtn = document.querySelector('button'); // 버튼 가져오기
const elPre = document.querySelector('ul');     // ul 가져오기
let url = "../data/person_card.json";           // 자료주소 가져오기

class dataInsert {               // dataInsrt라는 이름의 객체 생성
  constructor (title, image){    // constructor : 생성자
...
const fnGetData = jsonData => {
  ...
  data.forEach( data => {
    ...
    const mkLi = document.createElement('li');             // li 만들기
...
const fnFetch = path => {    // path = url = "../data/person_card.json";
  fetch(path)                // path에서 data를 받아 then으로 넘김
  .then( response => {       // 넘겨받은 data 일부 정제해서 다음 then으로 넘기는 내용을 담음

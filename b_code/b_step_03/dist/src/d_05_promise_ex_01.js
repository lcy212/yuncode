...
const fnRel = function(score, score2){
  const _n = parseInt(score);           // 숫자로 변환
  const nanCheck = isNaN(_n) === false; // _n은 NaN이 아니다 
  const numArea = _n >= 0 && _n <= 5;   // n이 0 ~5일 떼
  let _n2 = score2 || 0; // 아래의 rel값이 들어갈 곳?

  const sendData = new Promise(function(resolve, reject){ // Promise는 생성자함수이기때문에 new를 붙여줘야함
    if(nanCheck && numArea){  // 값이 NaN이 아니고 0~5일 때
      const nRel = _n + _n2;  // 입력값 + 누적값
      resolve(nRel);
    }  else {
...
const fnSubject = function(n, i, j){
  fnRel(n)                                 // 국어점수 입력받음 -> n 조건판단 후 맞다면 then으로, 거짓이면 catch로
    .then(function(rel1){                  // 값을 전달 받았을때 (ok) 
      ...
      const numResult = (rel3 + 5 ) * 5;   // 최종점수 = ((국어+영어+수학)+5)*5
      result.innerHTML = numResult;        // 최종점수를 html에 표시한다
    })
    .catch(function(error){                // 값에 에러가 났을때 (reject)
